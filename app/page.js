...
export default async function Home() {
  ...
  return (
    <>
      <Navbar />
      <div style={{ background: 'white', borderBottom: '1px solid var(--border)', padding: '64px 2rem 56px', textAlign: 'center' }}>
        <div style={{
        ...
        <h1 style={{
          ...
          fontWeight: 800, color: 'var(--ink)', lineHeight: 1.1,
          marginBottom: 16, letterSpacing: '-0.02em'
        }}>
          Something broken?<br /><span style={{ color: 'var(--teal)' }}>We can fix it.</span>
        </h1>

        <p style={{ fontSize: 17, color: 'var(--muted)', maxWidth: 500, margin: '0 auto 36px', fontWeight: 300 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 16 }}>
          <strong style={{ color: 'var(--ink)' }}>{postsWithCount.length}</strong> fix requests posted
        </div>
      </div>

      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 2rem 80px' }}>
        <div style={{ marginBottom: 16, fontSize: 13, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--muted)' }}>
          Browse by category
        </div>
          ...
          {CATEGORIES.map(c => (
            <a key={c.name} href={`/search?q=${c.name}`} style={{ textDecoration: 'none' }}>
              <div style={{
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--teal)'; e.currentTarget.style.background = 'var(--teal-light)'; e.currentTarget.style.transform = 'translateY(-2px)' }}
      ...
      <footer style={{ background: 'white', borderTop: '1px solid var(--border)', padding: '32px 2rem', textAlign: 'center' }}>
        <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, color: 'var(--teal)', marginBottom: 8 }}>FixThis</div>
        <div style={{ fontSize: 13, color: 'var(--muted)' }}>Crowd-sourced repair guides for everyday broken things.</div>
      </footer>
    </>
  )
}
